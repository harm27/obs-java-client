package nl.harm27.obswebsocket.api.events.scenes;

import nl.harm27.obswebsocket.api.events.BaseEvent;

/**
 * Triggered when a scene collection is created, added, renamed, or removed.
 *
 * @see <a href="https://github.com/Palakis/obs-websocket/blob/4.x-current/docs/generated/protocol.md#SceneCollectionListChanged">OBS WebSocket Documentation</a>
 * @since v4.0.0
 */
public class SceneCollectionListChanged extends BaseEvent {
}